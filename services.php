	<?php
		include "./include/head.php";
		include "./include/header.php";
	?>
	
	<link href="css/services.css" rel="stylesheet">

	<section class="hero services">
		<div class="container">
			<div class="row">
				<div class="col-md-8">
					<div class="content">
						<h3>
							We offer Best <b>Services</b><br>
							to help you grow your business
						</h3>

						<a href="#" class="btn btn-heroLink">
							Request a quote
						</a>
					</div>
				</div>
			</div>
		</div>
	</section>
	
	<section class="approach">
		<div class="container">
			<div class="row">
				<div class="col-md-6"></div>

				<div class="col-md-6">
					<div class="content">
						<div class="inbox">
							<h4>Our Approach</h4>

							<p>
								The basic approach which is used by me in the business aligns with the 
								provision of right and reputed business solutions to handle the best 
								idea to make the work proficient in different domains of life. Our vision 
								is production of a prime eco system which is based on new 
								opportunities for revenue progression and profitability. 
							</p>

							<a href="#" class="btn btn-request">
								REQUEST A QUOTE
							</a>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>

	<section class="award-winning">
		<div class="container">
			<h3>
				Our award-winning team provides the industry's best service and support for:
			</h3>

			<div class="mergeBox">
				<div class="row">
					<div class="col-md-4">
						<div class="icoBox">
							<figure>
								
								<img src="images/merge-img1.jpg" alt="">
							</figure>

							<h4>
							FUNDING <br>SOLUTION
							</h4>
						</div>
					</div>

					<div class="col-md-8">
						<div class="content">
							<p>
								We help dealmakers prepare for due diligence on their biggest 
								and most confidential transactions. When you choose Intralinks 
								for <b>your next deal,</b> you can expect:
							</p>

							
							<ul>
								<li>
									A dedicated <b>VDR</b>Pro expert that will help your deal team 
									with training and best practices to accelerate the preparation 
									and launch of your data room
								</li>

								<li>
									Assistant every step of the way - from deal launch and 
									due diligence to closing and archiving your data room
								</li>

								<li>
									A help desk that your team and all participants in your 
									data room can contact via phone. live chat or email, 7 
									days a week, 24 hours a day
								</li>
							</ul>
						</div>
					</div>
				</div>
			</div>

			<div class="mergeBox">
				<div class="row">
					<div class="col-md-4">
						<div class="icoBox">
							<figure>
								<img src="images/merge-img2.jpg" alt="">
							</figure>

							<h4>
							MARKETING <br>SOLUTION
							</h4>
						</div>
					</div>

					<div class="col-md-8">
						<div class="content">
							<p>
								We help alternative investment firms raise billion-dollar funds 
								and roll out <a href="#">investor portals</a> for their high-profile 
								LPs. When you choose Intralinks for <b>fundraising, fund reporting</b> 
								or <b>Portfolio Company management</b>, you can expect.
							</p>

							
							<ul>
								<li>
									A dedicated expert with years of experience working with 
									alternative investments firms
								</li>

								<li>
									Best practices on setting up indexes, private LP folders, 
									K1 distribution processes, etc., based on our experience 
									working with hundreds of GPs and supporting some of the 
									biggest and most demanding LPs around the globe 
								</li>

								<li>
									A help desk that your team and all participants in your 
									data room can contact via phone. live chat or email, 7 
									days a week, 24 hours a day
								</li>
							</ul>
						</div>
					</div>
				</div>
			</div>

			<div class="mergeBox">
				<div class="row">
					<div class="col-md-4">
						<div class="icoBox">
							<figure>
								<img src="images/merge-img3.jpg" alt="">
							</figure>

							<h4>
							Technology <br> SOLUTION
							</h4>
						</div>
					</div>

					<div class="col-md-8">
						<div class="content">
							<p>
								We help banking and securities firms secure how they 
								exchange files with each other and their clients. When 
								you select Intralinks as your <b>technology partner</b> 
								you can expect.
							</p>
							
							<ul>
								<li>
									Dedicated experts in client onboarding, debt capital 
									markets and investor communications use cases 
								</li>

								<li>
									Professional services options to help you integrate 
									Intralinks into your environment or create custom 
									solutions leveraging Intralinks platform and security
								</li>

								<li>
									Option to set up dedicated help desks for deployments 
									supporting business critical processes 
								</li>
							</ul>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>

	<section class="goodCompany">
		<div class="container">
			<h3>You're in good company.</h3>

			<p>
				Powering Small Business Growth across the industry
			</p>

			<div class="row">
				<div class="col-md-12">
					<!-- Nav tabs -->
					<ul class="nav nav-tabs">
						<li class="nav-item">
							<a class="nav-link active" data-toggle="tab" href="#good1">Real Estate</a>
						</li>

						<li class="nav-item">
							<a class="nav-link" data-toggle="tab" href="#good2">Media & Entertainment</a>
						</li>

						<li class="nav-item">
							<a class="nav-link" data-toggle="tab" href="#good3">Fitness</a>
						</li>

						<li class="nav-item">
							<a class="nav-link" data-toggle="tab" href="#good4">Gaming</a>
						</li>

						<li class="nav-item">
							<a class="nav-link" data-toggle="tab" href="#good5">Energy</a>
						</li>

						<li class="nav-item">
							<a class="nav-link" data-toggle="tab" href="#good6">Logistics</a>
						</li>
					</ul>

					<!-- Tab panes -->
					<div class="tab-content">
						<div class="tab-pane container active" id="good1">
							<div class="contentBox">
								<figure>
									<img src="images/good-company-img.jpg" alt="">
								</figure>
							</div>
						</div>

						<div class="tab-pane container fade" id="good2">
							<div class="contentBox">
								<figure>
									<img src="images/good-company-img.jpg" alt="">
								</figure>
							</div>
						</div>

						<div class="tab-pane container fade" id="good3">
							<div class="contentBox">
								<figure>
									<img src="images/good-company-img.jpg" alt="">
								</figure>
							</div>
						</div>

						<div class="tab-pane container fade" id="good4">
							<div class="contentBox">
								<figure>
									<img src="images/good-company-img.jpg" alt="">
								</figure>
							</div>
						</div>

						<div class="tab-pane container fade" id="good5">
							<div class="contentBox">
								<figure>
									<img src="images/good-company-img.jpg" alt="">
								</figure>
							</div>
						</div>

						<div class="tab-pane container fade" id="good6">
							<div class="contentBox">
								<figure>
									<img src="images/good-company-img.jpg" alt="">
								</figure>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>

	<section class="assessment">
		<div class="container">
			<h3>
				Connect with Us for a your companies Business Growth Assessment<br>
				& Growth Recommendations.
			</h3>

			<div class="row">
				<div class="col-md-3">
					<div class="connectBox">
						<figure>
							<img src="images/assesment-ico1.png" alt="">
						</figure>

						<h4>
							Funding & Investment Solutions<br>
							for your business
						</h4>
					</div>
				</div>
				
				<div class="col-md-3">
					<div class="connectBox">
						<figure>
							<img src="images/assesment-ico2.png" alt="">
						</figure>

						<h4>
							Marketing Solution<br>
							for your business
						</h4>
					</div>
				</div>
				
				<div class="col-md-3">
					<div class="connectBox">
						<figure>
							<img src="images/assesment-ico3.png" alt="">
						</figure>

						<h4>
							Automation & Technology<br>
							Solutions for your Business
						</h4>
					</div>
				</div>
				
				<div class="col-md-3">
					<div class="connectBox">
						<figure>
							<img src="images/assesment-ico4.png" alt="">
						</figure>

						<h4>
							Business & Growth<br>
							Assessment
						</h4>
					</div>
				</div>
			</div>

			<a href="#" class="btn btn-self">
				Download Self-Diagnostics<br>
				& Assesment
			</a>
		</div>
	</section>

	<section class="logosComp d-none">
		<div class="container">
			<h3>Partners we bring on bord to your Business</h3>
			
			<div class="company slider" data-sizes="50vw">
				<div>
					<img data-lazy="./images/customer-logo1.png" data-srcset="./images/customer-logo1.png" >
				</div>

				<div>
					<img data-lazy="./images/customer-logo2.png" data-srcset="./images/customer-logo2.png" >
				</div>

				<div>
					<img data-lazy="./images/customer-logo3.png" data-srcset="./images/customer-logo3.png" >
				</div>

				<div>
					<img data-lazy="./images/customer-logo4.png" data-srcset="./images/customer-logo4.png" >
				</div>

				<div>
					<img data-lazy="./images/customer-logo5.png" data-srcset="./images/customer-logo5.png" >
				</div>
			</div>

			<a href="#" class="btn btn-logos ">
				SEE ALL Customers
			</a>
		</div>
	</section>

	<section class="faq">
		<div class="container">
			<div class="row">
				<div class="col-md-12">
					<div class="faqDv">
						<div id="accordion">
							<div class="card">
								<div class="card-header" id="headingOne">
									<button class="btn btn-link" data-toggle="collapse" data-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
										<h5>
										What makes growthbay different?
										</h5>
									</button>
								</div>

								<div id="collapseOne" class="collapse show" aria-labelledby="headingOne" data-parent="#accordion">
									<div class="card-body">
									Having a great team of experienced staffs who fully understand your business needs, constrains and the best way to make your business growth less tedious we cannot be compared to just any other establishment.
									</div>
								</div>
							</div>

							<div class="card">
								<div class="card-header" id="headingTwo">
									<button class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">
										<h5>
										How much work is necessary to solve my challenge?
										</h5>
									</button>
								</div>

								<div id="collapseTwo" class="collapse" aria-labelledby="headingTwo" data-parent="#accordion">
									<div class="card-body">
									We are ready to work with you every step of the way,which makes it easy,so be rest assured the process will be worth it.
									</div>
								</div>
							</div>

							<div class="card">
								<div class="card-header" id="headingThree">
									<button class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseThree" aria-expanded="false" aria-controls="collapseThree">
										<h5>What documents are necessary to process a small business funding?</h5>
									</button>
								</div>

								<div id="collapseThree" class="collapse" aria-labelledby="headingThree" data-parent="#accordion">
									<div class="card-body">
									fill our inquiry form or book a call with us today.
									</div>
								</div>
							</div>

							<div class="card">
								<div class="card-header" id="headingFour">
									<button class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseFour" aria-expanded="false" aria-controls="collapseThree">
										<h5>
											If we become partners,who owns majority of the companny?
										</h5>
									</button>
								</div>
								
								<div id="collapseFour" class="collapse" aria-labelledby="headingFour" data-parent="#accordion">
									<div class="card-body">
									This depends on our agreement terms.
									</div>
								</div>
							</div>

							<div class="card">
								<div class="card-header" id="headingFive">
									<button class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseFive" aria-expanded="false" aria-controls="collapseThree">
										<h5>What is the max. amount of fund that can be provided?</h5>
									</button>
								</div>
								
								<div id="collapseFive" class="collapse" aria-labelledby="headingFive" data-parent="#accordion">
									<div class="card-body">
									500,000dollars
									</div>
								</div>
							</div>

							<div class="card">
								<div class="card-header" id="headingSix">
									<button class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseSix" aria-expanded="false" aria-controls="collapseThree">
										<h5>Do you only provide marketing consultation or you implement them?</h5>
									</button>
								</div>
								
								<div id="collapseSix" class="collapse" aria-labelledby="headingSix" data-parent="#accordion">
									<div class="card-body">
									Yes, we also help with implementation
									</div>
								</div>
							</div>
							
							<div class="card">
								<div class="card-header" id="headingSeven">
									<button class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseSeven" aria-expanded="false" aria-controls="collapseThree">
										<h5>
											For technology;do you offer staff training as well as installations?
										</h5>
									</button>
								</div>
								
								<div id="collapseSeven" class="collapse" aria-labelledby="headingSeven" data-parent="#accordion">
									<div class="card-body">
									We offer staff trainings on how to use softwares and other products. We also do installations.
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>

	<section class="newsletter">
		<div class="container">
			<div class="row">
				<div class="col-md-4">
					<div class="newsLbox">
						<div class="ico">
							<figure>
								<img src="images/news-img.jpg" alt="">
							</figure>
						</div>

						<h4>
							GET OUR FREE<br>
							NEWSLETTER & REAL<br>
							ESTATE INFORMATION
						</h4>
					</div>
				</div>

				<div class="col-md-8">
					<form action="#" class="news">
						<div class="row">
							<div class="col-md-9">
								<div class="row">
									<div class="col-md-4">
										<div class="form-group">
											<input type="text" class="form-control" placeholder="First Name *">
										</div>
									</div>
									
									<div class="col-md-4">
										<div class="form-group">
											<input type="text" class="form-control" placeholder="Last Name *">
										</div>
									</div>
									
									<div class="col-md-4">
										<div class="form-group">
											<input type="text" class="form-control" placeholder="Email Address *">
										</div>
									</div>
 
									<div class="col-md-8">
										<div class="form-group dDown">
											<select name="des" id="des" class="describe">
												<option value="">Which best describes you? *</option>
												<option value="1">1</option>
												<option value="2">2</option>
											</select>
										</div>
									</div>
									
									<div class="col-md-4">
										<div class="form-group">
											<input type="text" class="form-control" placeholder="Phone">
										</div>
									</div>
								</div>
							</div>

							<div class="col-md-3">
								<button class="btn btn-signup">SIGNUP</button>
							</div>
						</div>
					</form>
				</div>
			</div>
		</div>
	</section>
	
	<section class="worth ">
		<div class="container">
			<div class="row">
				<div class="col-md-4">
					<div class="worthBox">
						<div class="wFlex">
							<div class="col-flex">
								<h4>
									4.1 million+
								</h4>

								<hr class="dvdr">

								<p>
									users from all corners of the
									globe
								</p>
							</div>

							<div class="col-flex">
								<figure>
									<img src="./images/footer-logo1.png" alt="">
								</figure>
							</div>
						</div>
					</div>
				</div>
				
				<div class="col-md-4 blue">
					<div class="worthBox">
						<div class="wFlex">
							<div class="col-flex">
								<h4>
									10,000 M&A deals
								</h4>

								<hr class="dvdr">

								<p>
									are closed on SS&C Intralinks
									each year
								</p>
							</div>

							<div class="col-flex">
								<figure>
									<img src="./images/footer-logo2.png" alt="">
								</figure>
							</div>
						</div>
					</div>
				</div>
				
				<div class="col-md-4">
					<div class="worthBox">
						<div class="wFlex">
							<div class="col-flex">
								<h4>
									$34.7 trillion worth
								</h4>

								<hr class="dvdr">

								<p>
									of financial transactions
								</p>
							</div>

							<div class="col-flex">
								<figure>
									<img src="./images/footer-logo3.png" alt="">
								</figure>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>

	<section class="location d-none">
		<div class="container">
			<div class="boxTop">
				<div class="row">
					<div class="col-md-12">
						<h4>Why Partner with us?</h4>

						<p>
							Our mode of operation is dynamic and we are 
							always steps ahead of the ordinary
						</p>
					</div>
				</div>
			</div>
			
			<div class="inbox">
				<div class="row">
					<div class="col-md-4">
						<div class="serBox">
							<figure>
								<img src="images/gb-service-icon4.png" alt="">
							</figure>

							<h4>
								Excellent<br>
								Customer Service
							</h4>
						</div>
					</div>
					
					<div class="col-md-4">
						<div class="serBox">
							<figure>
								<img src="images/gb-service-icon5.png" alt="">
							</figure>

							<h4>
								Good Reviews<br>
								From Past Customers
							</h4>
						</div>
					</div>
					
					<div class="col-md-4">
						<div class="serBox">
							<figure>
								<img src="images/gb-service-icon6.png" alt="">
							</figure>

							<h4>
								Professional<br>
								consultation team
							</h4>
						</div>
					</div>
				</div>
			</div>

			<br>
			<br>
			<br>

			<div class="boxTop" style="margin-bottom: 0px;">
				<div class="row">
					<div class="col-md-12">
						<h5>
							We're Ready to Get Started
						</h5>

						<a href="#" class="btn btn-contact">
							CONTACT US
						</a>
					</div>
				</div>
			</div>
		</div>
	</section>

	<section class="GrowthBay d-none">
		<div class="container">
			<h3>
				What’s new at GrowthBay
			</h3>

			<div class="row">
				<div class="col-md-4">
					<div class="gbBox new">
						<div class="top">
							<p>NEW</p>
						</div>

						<div class="gbImg">
							<figure>
								<img src="./images/growthbay_img4.png" alt="">
							</figure>
						</div>
						
						<div class="gbBtnBox">
							<a href="#" class="btn btngbLink">
							Webinar
							</a>
						</div>

						<div class="gbContent">
							<h4>
							Webinar on How to expand into Europe
							</h4>

							<a href="#" class="watchNow">Watch NOW</a>
						</div>
					</div>
				</div>
				
				<div class="col-md-4">
					<div class="gbBox">
						<div class="top">
							<p>NEW</p>
						</div>

						<div class="gbImg">
							<figure>
								<img src="./images/growthbay_img2.png" alt="">
							</figure>
						</div>
						
						<div class="gbBtnBox">
							<a href="#" class="btn btngbLink orange">
								Virtual Event
							</a>
						</div>

						<div class="gbContent">
							<h4>
							Forbes/ EY Small Business Webinar
							</h4>

							<a href="#" class="watchNow">Register Now</a>
						</div>
					</div>
				</div>
				
				<div class="col-md-4">
					<div class="gbBox">
						<div class="top">
							<p>NEW</p>
						</div>

						<div class="gbImg">
							<figure>
								<img src="./images/growthbay_img3.png" alt="">
							</figure>
						</div>
						
						<div class="gbBtnBox">
							<a href="#" class="btn btngbLink purple">
							Service Update 
							</a>
						</div>

						<div class="gbContent">
							<h4>
							Technology update from Uipath
							</h4>

							<a href="#" class="watchNow">Learn More</a>
						</div>
					</div>
				</div>
			</div>

			<br>

			<p class="textBottom d-none">
				Scroll for Knowledge Base, Market Insights, Partners Proposals/Free Bees
			</p>
		</div>
	</section>

	<section class="connect d-none">
		<div class="container">
			<div class="row">
				<div class="col-md-4">
					<h3>
						More than two decades after 
						pioneering the virtual data 
						room, we’re still the leading 
						innovator in financial technology, 
						trusted by more than 99% of 
						the Fortune 1000.          
					</h3>

					<a href="#" class="btn btn-connect">
						See All Products
					</a>
				</div>

				<div class="col-md-8">
					<div class="row">
						<div class="col-md-4">
							<div class="cnBox">
								<div class="icoBox">
									<img src="./images/connect-img1.png" alt="">
								</div>

								<h4>
									Funding & Investment Solutions for your business
								</h4>
							</div>
						</div>
						
						<div class="col-md-4">
							<div class="cnBox">
								<div class="icoBox">
									<img src="./images/connect-img2.png" alt="">
								</div>

								<h4>
									Marketing Solution for your business
								</h4>
							</div>
						</div>
						
						<div class="col-md-4">
							<div class="cnBox">
								<div class="icoBox">
									<img src="./images/connect-img3.png" alt="">
								</div>

								<h4>
									Automation & Technology Solutions for your Business
								</h4>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>

	<section class="howWorks d-none">
		<div class="container">
			<h3>Here's How It Works:</h3>

			<div class="row">
				<div class="col-md-7">
					<div class="row">
						<div class="col-md-4">
							<div class="infoBox">
								<div class="count">
									<h2>1</h2>
								</div>

								<div class="title">
									<h3>
										Submit your Expression of Interest (EOI).
									</h3>
								</div>

								<div class="imagBox">
									<figure>
										<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
											width="440.001px" height="440.001px" viewBox="0 0 440.001 440.001" style="enable-background:new 0 0 440.001 440.001;"
											xml:space="preserve">
											<g>
												<g>
													<g>
														<path fill="#20bf55" d="M439.213,382.709l-27.329-109.314c-0.425-1.705-1.093-3.297-1.94-4.76c1.562-4.334,2.42-9.004,2.42-13.869V74.571
															c0-22.595-18.382-40.979-40.979-40.979H68.601c-22.599,0-40.981,18.383-40.981,40.979v180.193c0,4.866,0.857,9.536,2.423,13.87
															c-0.849,1.464-1.52,3.054-1.945,4.759L0.548,383.589c-1.371,5.486-0.138,11.3,3.343,15.756c3.48,4.458,8.82,7.064,14.477,7.064
															h403.249c0.006,0,0.013,0,0.017,0c10.146,0,18.367-8.225,18.367-18.367C440,386.188,439.725,384.397,439.213,382.709z
															M266.537,377.695c-0.004,0-0.009,0.002-0.016,0h-93.061c-1.074,0-2.094-0.477-2.781-1.301c-0.689-0.822-0.978-1.91-0.787-2.969
															l7.252-40.245c0.311-1.728,1.814-2.982,3.568-2.982h78.559c1.754,0,3.257,1.256,3.566,2.982l7.192,39.923
															c0.085,0.309,0.131,0.632,0.131,0.966C270.163,376.073,268.539,377.695,266.537,377.695z M376.57,254.766
															c0,2.094-1.25,3.897-3.04,4.717H66.452c-1.792-0.818-3.043-2.623-3.043-4.717V74.571c0-2.861,2.329-5.188,5.191-5.188h302.783
															c2.859,0,5.188,2.327,5.188,5.188L376.57,254.766L376.57,254.766z"/>
														<path fill="#20bf55" d="M229.972,157.081l11.472-3.129c2.271-0.619,3.889-2.631,4-4.984c0.114-2.353-1.302-4.511-3.502-5.346l-52.898-20.064
															c-2-0.758-4.26-0.273-5.773,1.239c-1.514,1.513-1.998,3.772-1.24,5.773l20.064,52.898c0.836,2.202,2.994,3.617,5.346,3.503
															c2.354-0.113,4.365-1.728,4.984-4l3.129-11.472l26.766,26.763c2.123,2.124,5.564,2.124,7.688,0l6.729-6.728
															c2.123-2.124,2.123-5.566,0-7.69L229.972,157.081z"/>
													</g>
												</g>
											</g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
										</svg>
									</figure>
								</div>

								<div class="desc">
									<p>
										Enter Your information in the web form above. 
									</p>
								</div>
							</div>
						</div>
						
						<div class="col-md-4">
							<div class="infoBox">
								<div class="count">
									<h2>2</h2>
								</div>

								<div class="title">
									<h3>
										Initial Call to discuss your 
										Business request and Growth Vision.
									</h3>
								</div>

								<div class="imagBox">
									<figure>
										<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
											viewBox="0 0 322 322" style="enable-background:new 0 0 322 322;" xml:space="preserve">
											<g id="XMLID_7_">
												<path fill="#20bf55" id="XMLID_8_" d="M275.445,135.123c0.387-45.398-38.279-87.016-86.192-92.771c-0.953-0.113-1.991-0.285-3.09-0.467
													c-2.372-0.393-4.825-0.797-7.3-0.797c-9.82,0-12.445,6.898-13.136,11.012c-0.672,4-0.031,7.359,1.902,9.988
													c3.252,4.422,8.974,5.207,13.57,5.836c1.347,0.186,2.618,0.359,3.682,0.598c43.048,9.619,57.543,24.742,64.627,67.424
													c0.173,1.043,0.251,2.328,0.334,3.691c0.309,5.102,0.953,15.717,12.365,15.717h0.001c0.95,0,1.971-0.082,3.034-0.244
													c10.627-1.615,10.294-11.318,10.134-15.98c-0.045-1.313-0.088-2.555,0.023-3.381C275.429,135.541,275.444,135.332,275.445,135.123z
													"/>
												<path fill="#20bf55" id="XMLID_9_" d="M176.077,25.688c1.275,0.092,2.482,0.18,3.487,0.334c70.689,10.871,103.198,44.363,112.207,115.605
													c0.153,1.211,0.177,2.688,0.202,4.252c0.09,5.566,0.275,17.145,12.71,17.385l0.386,0.004c3.9,0,7.002-1.176,9.221-3.498
													c3.871-4.049,3.601-10.064,3.383-14.898c-0.053-1.186-0.104-2.303-0.091-3.281C318.481,68.729,255.411,2.658,182.614,0.201
													c-0.302-0.01-0.59,0.006-0.881,0.047c-0.143,0.021-0.408,0.047-0.862,0.047c-0.726,0-1.619-0.063-2.566-0.127
													C177.16,0.09,175.862,0,174.546,0c-11.593,0-13.797,8.24-14.079,13.152C159.817,24.504,170.799,25.303,176.077,25.688z"/>
												<path fill="#20bf55" id="XMLID_10_" d="M288.36,233.703c-1.503-1.148-3.057-2.336-4.512-3.508c-7.718-6.211-15.929-11.936-23.87-17.473
													c-1.648-1.148-3.296-2.297-4.938-3.449c-10.172-7.145-19.317-10.617-27.957-10.617c-11.637,0-21.783,6.43-30.157,19.109
													c-3.71,5.621-8.211,8.354-13.758,8.354c-3.28,0-7.007-0.936-11.076-2.783c-32.833-14.889-56.278-37.717-69.685-67.85
													c-6.481-14.564-4.38-24.084,7.026-31.832c6.477-4.396,18.533-12.58,17.679-28.252c-0.967-17.797-40.235-71.346-56.78-77.428
													c-7.005-2.576-14.365-2.6-21.915-0.06c-19.02,6.394-32.669,17.623-39.475,32.471C2.365,64.732,2.662,81.578,9.801,99.102
													c20.638,50.666,49.654,94.84,86.245,131.293c35.816,35.684,79.837,64.914,130.839,86.875c4.597,1.978,9.419,3.057,12.94,3.844
													c1.2,0.27,2.236,0.5,2.991,0.707c0.415,0.113,0.843,0.174,1.272,0.178l0.403,0.002c0.001,0,0,0,0.002,0
													c23.988,0,52.791-21.92,61.637-46.91C313.88,253.209,299.73,242.393,288.36,233.703z"/>
												<path fill="#20bf55" id="XMLID_11_" d="M186.687,83.564c-4.107,0.104-12.654,0.316-15.653,9.021c-1.403,4.068-1.235,7.6,0.5,10.498
													c2.546,4.252,7.424,5.555,11.861,6.27c16.091,2.582,24.355,11.48,26.008,28c0.768,7.703,5.955,13.082,12.615,13.082h0.001
													c0.492,0,0.995-0.029,1.496-0.09c8.01-0.953,11.893-6.838,11.542-17.49c0.128-11.117-5.69-23.738-15.585-33.791
													C209.543,88.98,197.574,83.301,186.687,83.564z"/>
											</g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
										</svg>
									</figure>
								</div>

								<div class="desc">
									<p>
										You will be contacted to discuss your business 
										request and our Offer framework.
									</p>
								</div>
							</div>
						</div>
						
						<div class="col-md-4">
							<div class="infoBox">
								<div class="count">
									<h2>3</h2>
								</div>

								<div class="title">
									<h3>
										Business Management Meetings 
										to Agree Growth plan.
									</h3>
								</div>

								<div class="imagBox">
									<figure>
										<svg id="bold" enable-background="new 0 0 24 24" height="512" viewBox="0 0 24 24" width="512" xmlns="http://www.w3.org/2000/svg">
											<path fill="#20bf55" d="m4.5 6.25v-4.5c0-.26.03-.51.1-.75h-2.85c-.96 0-1.75.79-1.75 1.75v4.5c0 .96.79 1.75 1.75 1.75h.37l1.28 1.7c.14.18.35.29.57.3h.03c.22 0 .42-.09.56-.26l1.5-1.71c-.93-.58-1.56-1.61-1.56-2.78z"/>
											<path fill="#20bf55" d="m22.25 1h-2.85c.07.24.1.49.1.75v4.5c0 1.17-.63 2.2-1.56 2.78l1.5 1.71c.14.17.34.26.56.26h.03c.22-.01.43-.12.57-.3l1.27-1.7h.38c.96 0 1.75-.79 1.75-1.75v-4.5c0-.96-.79-1.75-1.75-1.75z"/>
											<path fill="#20bf55" d="m12 10c-.009 0-.019 0-.028 0-.226-.009-.436-.119-.572-.3l-1.275-1.7h-2.375c-.965 0-1.75-.785-1.75-1.75v-4.5c0-.965.785-1.75 1.75-1.75h8.5c.965 0 1.75.785 1.75 1.75v4.5c0 .965-.785 1.75-1.75 1.75h-2.16l-1.526 1.744c-.142.163-.348.256-.564.256z"/>
											<circle fill="#20bf55" cx="4" cy="15" r="2"/>
											<path fill="#20bf55" d="m6.67 18.4c-1.01.76-1.67 1.98-1.67 3.35v.25h-4.25c-.41 0-.75-.34-.75-.75v-.5c0-1.52 1.23-2.75 2.75-2.75h2.5c.52 0 1.01.15 1.42.4z"/>
											<circle fill="#20bf55" cx="20" cy="15" r="2"/>
											<path fill="#20bf55" d="m24 20.75v.5c0 .41-.34.75-.75.75h-4.25v-.25c0-1.37-.66-2.59-1.67-3.35.41-.25.9-.4 1.42-.4h2.5c1.52 0 2.75 1.23 2.75 2.75z"/>
											<circle fill="#20bf55" cx="12" cy="14.5" r="3"/>
											<path fill="#20bf55" d="m14.75 19h-5.5c-1.517 0-2.75 1.233-2.75 2.75v1.5c0 .414.336.75.75.75h9.5c.414 0 .75-.336.75-.75v-1.5c0-1.517-1.233-2.75-2.75-2.75z"/>
										</svg>
									</figure>
								</div>

								<div class="desc">
									<p>
										This will also include Document exchange, 
										Legal protections (NDAs), and involvement of 
										GrowthBay partners. 
									</p>
								</div>
							</div>
						</div>
					</div>
				</div>

				<div class="col-md-5">
					<div class="row">
						<div class="col-md-6">
							<div class="infoBox">
								<div class="count">
									<h2>4</h2>
								</div>

								<div class="title">
									<h3>
										“Offer Proposal”
									</h3>
								</div>

								<div class="imagBox">
									<figure>
										<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
											viewBox="0 0 512.001 512.001" style="enable-background:new 0 0 512.001 512.001;" xml:space="preserve">
											<g>
												<g>
													<path fill="#20bf55" d="M504.501,187.277c-4.143,0-7.5,3.357-7.5,7.5v18.475c0,7.984-6.496,14.481-14.481,14.481
														c-8.08,0-14.482-6.628-14.482-14.481c0.028-114.853-0.066-107.866,0.073-109.267c0.716-7.205,6.825-13.05,14.41-13.05
														c7.984,0,14.481,6.496,14.481,14.481v54.358c-0.002,4.143,3.355,7.501,7.498,7.501c4.143,0,7.5-3.357,7.5-7.5v-54.359
														c0-16.257-13.226-29.482-29.482-29.482c-12.079,0-22.479,7.303-27.03,17.725c-7.44-0.631-17.363,2.274-28.066-5.66
														c-13.617-10.098-30.634-15.889-46.685-15.889l-96.292,0.001c-23.526,0-35.747,27.545-20.444,45.177
														c-11.927,9.325-13.916,26.33-4.824,38.087c-0.078,0.1-15.358,9.181-11.938,27.708c1.027,5.649,3.754,10.623,7.595,14.433
														c-18.376,16.6-6.663,47.322,18.19,47.322h50.598c-17.674,32.319-16.502,64.987-16.295,68.817
														c1.58,29.247,35.414,39.715,49.492,13.898c3.776-6.925,5.022-15.144,6.226-22.962c2.632-10.81,10.651-30.265,34.886-41.765
														c8.693-2.797,17.38-7.34,25.156-13.159c5.602-4.193,10.199-9.854,13.295-16.369c5.128-10.796,17.077-8.031,19.044-8.423
														c4.519,10.493,14.96,17.859,27.092,17.859c16.257,0,29.482-13.226,29.482-29.482v-18.475
														C512.001,190.634,508.644,187.277,504.501,187.277z M449.709,209.874c-11.426,0-21.975,6.668-26.877,16.987
														c-2.054,4.323-5.074,8.057-8.734,10.796c-11.26,8.427-20.75,10.754-22.085,11.376c-30.502,14.242-40.427,38.924-43.63,52.369
														c-0.5,2.093-1.458,12.963-4.734,18.97c-6.901,12.66-20.594,6.332-21.342-7.526c-0.194-3.592-1.348-35.967,17.682-66.275
														c4.542-7.234-0.645-16.734-9.267-16.734h-57.698c-6.707,0-12.164-5.456-12.164-12.163c0-6.707,5.456-12.164,12.164-12.164
														c16.668-0.034-4.538-0.104,7.692-0.104c4.143,0,7.5-3.357,7.5-7.5s-3.357-7.5-7.5-7.5h-6.709c-5.394,0-10.664-3.533-12.025-10.097
														c-0.773-4.397,1.221-7.585,1.53-8.367l1.248-1.726l1.188-1.198c0.636-0.551,2.203-1.559,2.103-1.491
														c2.334-0.9,3.43-1.551,5.955-1.551h6.708c4.142,0,7.5-3.357,7.5-7.5c0-4.142-3.357-7.5-7.499-7.5
														c-13.096-0.002-17.006-18.134-4.891-23.41c3.221-1.405,5.358-0.923,8.617-1.021c4.143,0,7.5-3.357,7.5-7.5
														c0-4.084-3.264-7.405-7.325-7.498c-1.375-0.027-0.652-0.026-1.485-0.073c-0.1-0.02-2.297-0.46-2.157-0.432l-0.623-0.202
														l-3.012-1.577c-0.102-0.086-1.677-1.427-1.575-1.341l-1.516-1.865c-0.048-0.109-2.484-3.492-1.932-8.161
														c0.916-6.621,6.269-10.781,12.128-10.781l96.292-0.001c12.713,0,26.825,4.836,37.75,12.939c15.16,11.239,29.562,8.06,34.547,8.611
														v101.214H449.709z"/>
												</g>
											</g>
											<g>
												<g>
													<path fill="#20bf55" d="M257.164,314.486c18.377-16.594,6.669-47.322-18.188-47.322h-50.598c17.674-32.319,16.502-64.987,16.295-68.818
														c-1.588-29.362-35.458-39.63-49.492-13.897c-3.776,6.925-5.021,15.144-6.226,22.962c-2.632,10.81-10.651,30.265-34.886,41.765
														c-8.692,2.796-17.378,7.338-25.157,13.159c-5.602,4.193-10.199,9.855-13.294,16.369c-5.128,10.796-17.077,8.031-19.044,8.424
														c-4.519-10.494-14.96-17.86-27.091-17.86C13.226,269.265,0,282.492,0,298.749v11.596c0,4.143,3.357,7.5,7.5,7.5s7.5-3.357,7.5-7.5
														v-11.596c0-7.985,6.496-14.482,14.482-14.482c8.146,0,14.482,6.704,14.482,14.482v107.836c0,7.804-6.359,14.481-14.482,14.481
														c-7.984,0-14.482-6.496-14.482-14.481v-61.238c0-4.143-3.357-7.5-7.5-7.5s-7.5,3.357-7.5,7.5v61.238
														c0,16.257,13.226,29.482,29.483,29.482c12.079,0,22.479-7.303,27.03-17.725c7.44,0.631,17.363-2.274,28.066,5.66
														c13.617,10.098,30.634,15.889,46.685,15.889h42.084c4.143,0,7.5-3.357,7.5-7.5s-3.357-7.5-7.5-7.5h-42.084
														c-12.713,0-26.825-4.836-37.75-12.939c-15.16-11.239-29.562-8.06-34.547-8.611V302.126h3.325c11.426,0,21.975-6.668,26.877-16.987
														c2.054-4.323,5.074-8.057,8.733-10.797c11.263-8.427,20.751-10.753,22.086-11.375c30.502-14.242,40.427-38.924,43.63-52.369
														c0.047-0.196,0.086-0.396,0.116-0.595c0.906-5.886,1.945-13.474,4.618-18.375c6.917-12.692,20.594-6.299,21.342,7.526
														c0.194,3.592,1.348,35.967-17.682,66.275c-4.542,7.234,0.645,16.734,9.267,16.734h57.698c6.707,0,12.164,5.456,12.164,12.163
														c0,6.706-5.456,12.163-12.164,12.163h-5.66c-0.26,0-0.52,0.014-0.778,0.04c-0.097,0.011-0.194,0.016-0.293,0.022l-0.326,0.025
														c-0.21,0.011-0.422,0.017-0.635,0.017c-4.143,0-7.5,3.357-7.5,7.5s3.357,7.5,7.5,7.5h6.709c5.394,0,10.665,3.533,12.026,10.098
														c1.016,5.69-2.365,9.466-2.411,9.646l-1.146,1.268c-0.101,0.093-0.511,0.471-0.41,0.378c-0.634,0.549-2.071,1.482-1.971,1.414
														c-3.716,2.138-7.489,1.624-12.796,1.628c-4.142,0-7.5,3.357-7.5,7.5c0,4.142,3.357,7.5,7.499,7.5
														c13.226,0.002,17.138,18.534,4.459,23.587c-3.22,1.255-5.326,0.712-8.185,0.844c-4.143,0-7.5,3.357-7.5,7.5
														c0,4.084,3.264,7.405,7.325,7.498c3.023,0.075,5.194,1.072,5.094,1.036c1.803,0.854,3.188,2.04,3.048,1.929
														c8.481,7.42,3.439,21.467-7.969,21.467h-19.206c-4.143,0-7.5,3.357-7.5,7.5s3.357,7.5,7.5,7.5h19.206
														c13.158,0,24.417-9.39,26.778-22.363c-0.039-0.167,2.972-12.098-6.333-22.813c11.962-9.352,13.893-26.356,4.825-38.087
														C267.145,347.383,269.618,326.833,257.164,314.486z"/>
												</g>
											</g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
											<g></g>
										</svg>
									</figure>
								</div>

								<div class="desc">
									<p>
										This will include Deal Structuring, 
										Due-diligence, and Negotiations.
									</p>
								</div>
							</div>
						</div>
						
						<div class="col-md-6">
							<div class="infoBox">
								<div class="count">
									<h2>5</h2>
								</div>

								<div class="title">
									<h3>
										Fund Disbursement & Strategic 
										Investment in your business
									</h3>
								</div>

								<div class="imagBox">
									<figure>
										<svg id="Layer_1" enable-background="new 0 0 512.004 512.004" height="512" viewBox="0 0 512.004 512.004" width="512" xmlns="http://www.w3.org/2000/svg">
											<g>
												<g>
													<path fill="#20bf55" d="m511.437 255.181-48.106-137.063c-1.65-4.702-6.51-7.47-11.397-6.494l-50.104 10.023-88.066-26.197c-12.711-3.782-26.64-6.031-42.346 3.038l-15.695 9.062-15.696-9.062c-15.707-9.069-29.636-6.819-42.346-3.037l-87.753 26.105-45.561-12.018c-4.954-1.306-10.098 1.344-11.907 6.139l-51.815 137.334c-1.729 4.582.115 9.742 4.356 12.19l43.077 24.87c-3.056 7.107-3.642 14.997-1.6 22.618 2.401 8.962 8.127 16.44 16.122 21.056 6.336 3.658 13.363 5.079 20.155 4.514.281 11.695 6.466 22.98 17.316 29.245 10.074 5.817 21.885 5.985 31.739 1.495 2.815 6.819 7.813 12.797 14.682 16.762 7.995 4.616 17.334 5.836 26.297 3.434 2.634-.706 5.141-1.699 7.483-2.953 2.977 6.413 7.825 11.741 14.05 15.334 5.446 3.144 11.397 4.638 17.274 4.638 8.319-.001 16.484-3 22.848-8.544l30.4 17.551c5.444 3.143 11.396 4.637 17.272 4.636 11.99 0 23.667-6.217 30.08-17.325 2.654-4.598 4.133-9.557 4.528-14.528 7.237 3.248 15.32 3.899 23.121 1.81 8.962-2.402 16.44-8.128 21.056-16.124 3.181-5.51 4.672-11.539 4.636-17.484 13.349 1.578 27.009-4.723 34.146-17.086 2.733-4.734 4.22-9.854 4.561-14.974 15.429 5.454 33.088-.684 41.562-15.359 3.676-6.367 5.096-13.428 4.51-20.251l55.192-20.68c5.097-1.909 7.731-7.542 5.929-12.675zm-438.836 61.245c-3.369-1.945-5.785-5.11-6.804-8.913-1.02-3.802-.509-7.751 1.437-11.121l13.962-24.184c1.945-3.369 5.11-5.786 8.913-6.804 3.802-1.019 7.752-.508 11.12 1.437 6.992 4.037 9.405 13.013 5.38 20.012l-13.983 24.218c-.001.002-.002.003-.003.005-4.048 6.987-13.029 9.387-20.022 5.35zm57.504 28.39c-4.044 7.001-13.027 9.41-20.033 5.368-7.003-4.043-9.41-13.03-5.369-20.032l5.229-9.056c.001-.002.002-.003.003-.004l13.994-24.238c.001-.002.003-.004.004-.006l.005-.01c.001-.001.001-.001.002-.002l12.345-21.384c1.945-3.369 5.11-5.785 8.912-6.804 3.804-1.019 7.751-.508 11.121 1.438 3.369 1.945 5.786 5.11 6.805 8.912s.509 7.751-1.437 11.12l-3.681 6.374c-.007.011-.013.022-.02.033-.001.001-.001.003-.002.004l-24.177 41.877c0 .001-.001.001-.001.002l-.002.002c-.001.003-.004.007-.005.01zm46.421 18.258c-1.945 3.368-5.109 5.784-8.912 6.803-3.804 1.019-7.753.508-11.121-1.437-6.995-4.039-9.405-13.012-5.379-20.012l.01-.017c.001-.001.001-.002.002-.002.002-.003.003-.007.005-.011l24.17-41.865c.002-.003.004-.005.005-.008 4.044-7.003 13.03-9.411 20.033-5.367 3.369 1.945 5.785 5.11 6.805 8.912 1.019 3.802.509 7.751-1.437 11.12l-5.107 8.846c-.001.001-.002.003-.003.004l-13.961 24.181c0 .001-.001.002-.002.003-.006.01-.012.02-.018.03zm27.798 21.182c-3.369-1.945-5.786-5.11-6.805-8.912s-.509-7.751 1.437-11.121l13.963-24.184c4.044-7.002 13.026-9.411 20.033-5.368 7.002 4.043 9.41 13.03 5.368 20.033l-13.963 24.185c-4.043 7.003-13.03 9.411-20.033 5.367zm90.554 8.278c-4.044 7.004-13.033 9.409-20.033 5.368l-28.778-16.615 9.573-16.582c1.771-3.068 3.016-6.298 3.768-9.584l30.104 17.381c.001 0 .001.001.002.001s.001 0 .002.001l.028.016c6.976 4.053 9.371 13.023 5.334 20.014zm148.085-122.6c-2.639-3.34-5.939-6.246-9.844-8.5-3.656-2.111-16.948-10.099-40.633-24.421-4.726-2.858-10.874-1.344-13.732 3.383-2.857 4.726-1.343 10.874 3.383 13.731 23.803 14.393 37.208 22.449 40.982 24.628 7.003 4.043 9.411 13.03 5.368 20.033-4.044 7.004-13.033 9.411-20.033 5.369l-21.425-12.371c-.006-.003-.013-.007-.019-.011l-.004-.002-79.582-45.946c-4.784-2.763-10.899-1.123-13.66 3.66-2.762 4.783-1.123 10.899 3.66 13.66l79.574 45.942c.001.001.003.002.004.003.002.001.002.001.004.002l.021.012c6.979 4.051 9.373 13.024 5.337 20.016-4.044 7.003-13.028 9.411-20.032 5.368l-14.082-8.13c-.009-.005-.017-.01-.025-.015l-.003-.001-69.864-40.336c-4.785-2.763-10.899-1.123-13.66 3.66-2.762 4.783-1.123 10.899 3.66 13.66l69.858 40.332c.001.001.002.001.003.002 6.999 4.044 9.404 13.029 5.363 20.031-1.945 3.369-5.11 5.785-8.912 6.804-3.801 1.019-7.752.509-11.121-1.436l-23.997-13.855c-.012-.007-.022-.013-.035-.021l-.002-.001-44.07-25.444c-2.933-4.981-7.142-9.297-12.495-12.388-6.385-3.686-13.462-5.105-20.305-4.505-.086-2.654-.479-5.32-1.185-7.954-2.401-8.962-8.128-16.44-16.123-21.056-6.87-3.966-14.552-5.306-21.862-4.333-.187-1.901-.531-3.8-1.036-5.684-2.401-8.962-8.128-16.44-16.124-21.057-14.665-8.468-33.01-4.964-43.648 7.452-2.855-4.268-6.702-7.96-11.439-10.695-7.995-4.616-17.336-5.837-26.297-3.435-8.963 2.401-16.44 8.127-21.057 16.123l-6.014 10.417-35.507-20.499 45.634-120.949 39.551 10.432c1.776.468 3.644.438 5.402-.084l90.442-26.905c10.523-3.13 17.885-3.868 26.643 1.188l5.695 3.289-35.032 20.226c-16.674 9.627-22.407 31.025-12.78 47.7 9.313 16.131 29.574 21.759 47.128 13.086 9.438-4.662 23.166-10.834 33.513-14.774 7.554 4.551 24.495 14.819 36.59 22.149 4.727 2.864 10.875 1.354 13.735-3.369 2.862-4.723 1.354-10.873-3.368-13.735-11.63-7.049-21.793-13.207-29.061-17.602-3.642-2.202-6.559-3.963-8.57-5.174-4.429-2.663-5.48-3.296-8.293-3.296-1.121 0-2.233.188-3.292.557-11.828 4.123-29.084 11.864-40.111 17.311-7.974 3.936-16.979 1.722-20.95-5.155-4.113-7.125-1.664-16.267 5.46-20.38l70.729-40.834c8.758-5.057 16.122-4.318 26.643-1.188l90.443 26.905c1.563.465 3.215.541 4.813.221l44.019-8.806 41.986 119.624z"/>
													<path fill="#20bf55" d="m348.815 212.262h-.008c-5.522 0-9.996 4.477-9.996 10s4.481 10 10.004 10 10-4.477 10-10-4.478-10-10-10z"/>
												</g>
											</g>
										</svg>
									</figure>
								</div>

								<div class="desc">
									<p>
										You will be contacted to discuss your business 
										request and our offer framework.
									</p>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>

	<section class="pre-qualification d-none">
		<div class="container">
			<h3>Pre-Qualification Criteria</h3>

			<div class="row">
				<div class="col-md-4">
					<div class="preBox">
						<div class="icoBox">
							<figure>
								<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
									viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
									<g>
										<g>
											<path fill="#fff" d="M488.727,279.273c-6.982,0-11.636,4.655-11.636,11.636v151.273c0,6.982-4.655,11.636-11.636,11.636H46.545
												c-6.982,0-11.636-4.655-11.636-11.636V290.909c0-6.982-4.655-11.636-11.636-11.636s-11.636,4.655-11.636,11.636v151.273
												c0,19.782,15.127,34.909,34.909,34.909h418.909c19.782,0,34.909-15.127,34.909-34.909V290.909
												C500.364,283.927,495.709,279.273,488.727,279.273z"/>
										</g>
									</g>
									<g>
										<g>
											<path fill="#fff" d="M477.091,116.364H34.909C15.127,116.364,0,131.491,0,151.273v74.473C0,242.036,11.636,256,26.764,259.491l182.691,40.727
												v37.236c0,6.982,4.655,11.636,11.636,11.636h69.818c6.982,0,11.636-4.655,11.636-11.636v-37.236l182.691-40.727
												C500.364,256,512,242.036,512,225.745v-74.473C512,131.491,496.873,116.364,477.091,116.364z M279.273,325.818h-46.545v-46.545
												h46.545V325.818z M488.727,225.745c0,5.818-3.491,10.473-9.309,11.636l-176.873,39.564v-9.309c0-6.982-4.655-11.636-11.636-11.636
												h-69.818c-6.982,0-11.636,4.655-11.636,11.636v9.309L32.582,237.382c-5.818-1.164-9.309-5.818-9.309-11.636v-74.473
												c0-6.982,4.655-11.636,11.636-11.636h442.182c6.982,0,11.636,4.655,11.636,11.636V225.745z"/>
										</g>
									</g>
									<g>
										<g>
											<path fill="#fff" d="M314.182,34.909H197.818c-19.782,0-34.909,15.127-34.909,34.909v11.636c0,6.982,4.655,11.636,11.636,11.636
												s11.636-4.655,11.636-11.636V69.818c0-6.982,4.655-11.636,11.636-11.636h116.364c6.982,0,11.636,4.655,11.636,11.636v11.636
												c0,6.982,4.655,11.636,11.636,11.636c6.982,0,11.636-4.655,11.636-11.636V69.818C349.091,50.036,333.964,34.909,314.182,34.909z"
												/>
										</g>
									</g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
								</svg>
							</figure>
						</div>

						<hr>

						<p>
							In Business more than 12 months
						</p>
					</div>
				</div>
				
				<div class="col-md-4">
					<div class="preBox">
						<div class="icoBox">
							<figure>
								<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" width="512" height="512">
									<g id="Layer_30" data-name="Layer 30">
										<path fill="#fff" d="M57,57H43V53H55a1,1,0,0,0,1-1V36a1,1,0,0,0-1-1H35V14a1,1,0,0,0-1-1H31V10a1,1,0,0,0-1-1H23V1H21V9H14a1,1,0,0,0-1,1v3H10a1,1,0,0,0-1,1V57H7a1,1,0,0,0-1,1v4a1,1,0,0,0,1,1H57a1,1,0,0,0,1-1V58A1,1,0,0,0,57,57ZM41,57H39V53h2ZM18,57V49h3v8Zm5-8h1v3a1,1,0,0,0,1,1h1v4H23Zm5,4h5v4H28Zm7,0h2v4H35Zm19-2H26V37H54ZM25,35a1,1,0,0,0-1,1v5H11V38.414L16.414,33h3.172l-2.293,2.293,1.414,1.414L22.414,33H33v2ZM18.707,22.707,22.414,19H33v8H11V24.414L16.414,19h3.172l-2.293,2.293ZM33,31H11V29H33ZM11,21.586V19h2.586ZM11,33h2.586L11,35.586Zm4-22H29v2H15Zm18,4v2H11V15ZM11,43H24v4H17a1,1,0,0,0-1,1v9H11ZM56,61H8V59H56Z"/>
										<rect fill="#fff" x="14.586" y="37" width="2.828" height="2" transform="translate(-22.184 22.444) rotate(-45)"/>
										<rect fill="#fff" x="14.586" y="23" width="2.828" height="2" transform="translate(-12.284 18.343) rotate(-45)"/>
										<path fill="#fff" d="M49,49h4V47H50V45h3V43H50V41h3V39H49a1,1,0,0,0-1,1v8A1,1,0,0,0,49,49Z"/>
										<path fill="#fff" d="M42,49h5V47H43V39H41v9A1,1,0,0,0,42,49Z"/>
										<path fill="#fff" d="M36,47h2v2h2V42a3,3,0,0,0-6,0v7h2Zm1-6a1,1,0,0,1,1,1v3H36V42A1,1,0,0,1,37,41Z"/>
										<path fill="#fff" d="M30,47a1,1,0,0,1-1-1H27a3,3,0,1,0,3-3,1,1,0,1,1,1-1h2a3,3,0,1,0-3,3,1,1,0,0,1,0,2Z"/>
									</g>
								</svg>
							</figure>
						</div>

						<hr>

						<p>
							Sales more than $10K per month
						</p>
					</div>
				</div>
				
				<div class="col-md-4">
					<div class="preBox">
						<div class="icoBox">
							<figure>
								<svg id="Layer_1_1_" enable-background="new 0 0 64 64" height="512" viewBox="0 0 64 64" width="512" xmlns="http://www.w3.org/2000/svg">
									<path fill="#fff" d="m21 29h5v2h-5z"/>
									<path fill="#fff" d="m8 29h11v2h-11z"/>
									<path fill="#fff" d="m8 10h2v2h-2z"/>
									<path fill="#fff" d="m8 33h18v2h-18z"/>
									<path fill="#fff" d="m8 37h18v2h-18z"/>
									<path fill="#fff" d="m8 41h18v2h-18z"/>
									<path fill="#fff" d="m21 14h5v2h-5z"/>
									<path fill="#fff" d="m8 14h11v2h-11z"/>
									<path fill="#fff" d="m21 18h5v2h-5z"/>
									<path fill="#fff" d="m8 18h11v2h-11z"/>
									<path fill="#fff" d="m8 25h2v2h-2z"/>
									<path fill="#fff" d="m60.879 22.536c.723-.724 1.121-1.685 1.121-2.707 0-2.112-1.718-3.829-3.828-3.829-1.023 0-1.984.398-2.707 1.122l-1.465 1.464v-2.586c0-2.757-2.243-5-5-5h-11v-4c0-2.757-2.243-5-5-5h-24c-2.757 0-5 2.243-5 5v47h-1c-.553 0-1 .448-1 1v2c0 2.757 2.243 5 5 5h19c2.206 0 4-1.794 4-4v-4.586l22-22v20.586h-8v2h8v2h-14v2h22v2h-28v2h29c.553 0 1-.448 1-1v-4c0-.552-.447-1-1-1h-7v-26.586zm-34.879 29.464v-2.586l28-28 2.586 2.586-28 28zm30.879-33.465c.341-.34.811-.535 1.293-.535 1.008 0 1.828.82 1.828 1.829 0 .488-.19.948-.535 1.293l-1.465 1.464-2.586-2.586zm-4.879-2.535v4.586l-22 22v-18.586h11c2.757 0 5-2.243 5-5v-3c0-1.654 1.346-3 3-3s3 1.346 3 3zm-6.974-3c-.635.838-1.026 1.87-1.026 3v3c0 1.654-1.346 3-3 3s-3-1.346-3-3v-6zm-9.026-6v12c0 1.13.391 2.162 1.026 3h-7.026v-15c0-1.654 1.346-3 3-3s3 1.346 3 3zm-29 53c-1.654 0-3-1.346-3-3v-1h18v2c0 .728.195 1.411.537 2zm19 0c-1.103 0-2-.897-2-2v-3c0-.552-.447-1-1-1h-17v-47c0-1.654 1.346-3 3-3h20.026c-.635.838-1.026 1.87-1.026 3v37.586l-3.707 3.707c-.188.187-.293.442-.293.707v4c0 .552.447 1 1 1h3v4c0 1.103-.897 2-2 2z"/>
								</svg>
							</figure>
						</div>

						<hr>

						<p>
							Founders’ commitment to business growth
						</p>
					</div>
				</div>
			</div>
		</div>
	</section>

	<section class="partner d-none">
		<div class="container">
			<div class="partnerBox">
				<h3>
					Partner of Choice for small businesses seeking national and international Markets
				</h3>

				<p>
					we can bring our partners and alliance network to get you new businesses 
				</p>

				<a href="#" class="btn btn-contact">
					CONTACT US
				</a>
			</div>
			
			<div class="pbBox">
				<div class="row">
					<div class="col-md-4">
						<div class="leftBox">
							<h4>Safer.</h4>

							<p>
								Share with confidence and compliance.
							</p>
						</div>

						<div class="leftBox">
							<h4>Simpler.</h4>

							<p>
								Focus on your deal, not the technology.
							</p>
						</div>

						<div class="leftBox">
							<h4>Smarter.</h4>

							<p>
								Save timeand avoid costly errors.
							</p>
						</div>
					</div>

					<div class="col-md-8">
						<div class="rightBox">
							<img src="./images/partner-right-img.png" alt="">
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>

	<section class="focus d-none">
		<div class="container">
			<div class="row">
				<div class="col-md-4">
					<div class="focus-box">
						<div class="icoBox">
							<figure>
								<img src="images/focus-box1.png" alt="">
							</figure>
						</div>

						<h4>
							Focus on your business goals
						</h4>
					</div>
				</div>
				
				<div class="col-md-4">
					<div class="focus-box">
						<div class="icoBox">
							<figure>
								<img src="images/focus-box2.png" alt="">
							</figure>
						</div>

						<h4>
							Leverage Partners
						</h4>
					</div>
				</div>
				
				<div class="col-md-4">
					<div class="focus-box">
						<div class="icoBox">
							<figure>
								<img src="images/focus-box3.png" alt="">
							</figure>
						</div>

						<h4>
							Compliant with Local and Business ethics, contracts etc
						</h4>
					</div>
				</div>
			</div>
		</div>
	</section>

	<section class="idsGroup d-none">
		<div class="container">
			<div class="idsContent">
				<h3>
					Interlynks Dedicated Services Group
				</h3>

				<p>
					Expert help, with a personal touch. On call anytime, 
					any place, in 140 languages.
				</p>

				<a href="#" class="btn btn-explore">
					EXPLORE YOUR OPTIONS
				</a>
			</div>
			
			<div class="single slider" data-sizes="50vw">
				<div>
					<img data-lazy="./images/ids-slide-img1.jpg" data-srcset="./images/ids-slide-img1.jpg" >
				</div>

				<div>
					<img data-lazy="./images/ids-slide-img1.jpg" data-srcset="./images/ids-slide-img1.jpg" >
				</div>
				<div>
					<img data-lazy="./images/ids-slide-img1.jpg" data-srcset="./images/ids-slide-img1.jpg" >
				</div>
			</div>
		</div>
	</section>

	<section class="insight d-none">
		<div class="container">
			<div class="row">
				<div class="col-md-3">
					<div class="insightBox">
						<div class="imgBox">
							<figure>
								<img src="./images/insights-img1.jpg" alt="">
							</figure>
						</div>

						<div class="contentBox">
							<h3>
								How Top M&A 
								Professionals Are 
								Embracing ESG in the 
								Deal Process
							</h3>

							<h4>
								WHITE PAPER
							</h4>
						</div>
					</div>
				</div>
				
				<div class="col-md-3">
					<div class="insightBox">
						<div class="imgBox">
							<figure>
								<img src="./images/insights-img2.jpg" alt="">
							</figure>
						</div>

						<div class="contentBox">
							<h3>
								How Top M&A 
								Professionals Are 
								Embracing ESG in the 
								Deal Process
							</h3>
						</div>
					</div>
				</div>
				
				<div class="col-md-3">
					<div class="insightBox">
						<div class="imgBox">
							<figure>
								<img src="./images/insights-img3.jpg" alt="">
							</figure>
						</div>

						<div class="contentBox">
							<h3>
								Asia-Pacific M&A 
								Activity Remains 
								Dynamic After A 
								Strong Start to the 
								Year
							</h3>
						</div>
					</div>
				</div>
				
				<div class="col-md-3">
					<div class="insightBox">
						<div class="imgBox">
							<figure>
								<img src="./images/insights-img4.jpg" alt="">
							</figure>
						</div>

						<div class="contentBox">
							<h3>
								SS&C Intralinks Deal 
								Flow Predictor for Q3 
								2021
							</h3>

							<h4>
								Publication
							</h4>
						</div>
					</div>
				</div>
			</div>

			<a href="#" class="btn btn-insights">
				see all insights
			</a>
		</div>
	</section>

	<?php
		include "./include/footer.php";
	?>